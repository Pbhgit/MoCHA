...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # self.evaclip_tower = build_vision_tower(config, load_model = "evaclip", delay_load=True)
            ...
            # self.evaclip_mm_projector = build_vision_projector(config)
    ...
    def get_vision_tower(self, load_model = "clip"):
        if load_model == "clip":
            ...
            vision_tower = getattr(self, 'evaclip_tower', None)
            if type(vision_tower) is list:
                vision_tower = vision_tower[0]
            return vision_tower 
        else:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # pretrain_evaclip_mm_mlp_adapter = model_args.pretrain_evaclip_mm_mlp_adapter
        ...
        if self.get_vision_tower(load_model='clip') is None:
            vision_tower = build_vision_tower(model_args)
            if fsdp is not None and len(fsdp) > 0:
                self.vision_tower = [vision_tower]
            else:
        ...
        # if self.get_vision_tower(load_model = "evaclip") is None:
        #     evaclip_tower = build_vision_tower(model_args, load_model="evaclip")

        #     if fsdp is not None and len(fsdp) > 0:
        #         self.evaclip_tower = [evaclip_tower]
        #     else:
        #         self.evaclip_tower = evaclip_tower
        # else:
        #     if fsdp is not None and len(fsdp) > 0:
        #         evaclip_tower = self.evaclip_tower[0]
        #     else:
        #         evaclip_tower = self.evaclipt_tower
        #     evaclip_tower.load_model()
        ...
        # self.config.hidden_size = getattr(model_args, 'hidden_size', '1280')
        ...
        # if getattr(self, 'evaclip_mm_projector', None) is None:
        #     self.evaclip_mm_projector = build_vision_projector(self.config)
        #     if 'unpad' in mm_patch_merge_type:
        #         embed_std = 1 / torch.sqrt(torch.tensor(self.config.hidden_size, dtype=self.dtype))
        #         self.image_newline = nn.Parameter(
        #             torch.randn(self.config.hidden_size, dtype=self.dtype) * embed_std
        ...
        # else:
        #     # In case it is frozen by LoRA
        #     for p in self.evaclip_mm_projector.parameters():
        #         p.requires_grad = True
        ...
        if pretrain_siglip_mm_mlp_adapter is not None:
            ...
            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
            self.siglip_mm_projector.load_state_dict(get_w(siglip_mm_projector_weights, 'siglip_mm_projector'))
        ...
        if pretrain_convnext_mm_mlp_adapter is not None:
            ...
            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
            self.convnext_mm_projector.load_state_dict(get_w(convnext_mm_projector_weights, 'convnext_mm_projector'))

        # if pretrain_evaclip_mm_mlp_adapter is not None:
        #     print("Loading pretrained evaclip adpater!!!")
        #     evaclip_mm_projector_weights = torch.load(pretrain_evaclip_mm_mlp_adapter, map_location='cpu')
        #     def get_w(weights, keyword):
        #         return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}
        #     self.evaclip_mm_projector.load_state_dict(get_w(evaclip_mm_projector_weights, 'evaclip_mm_projector'))
...
class LlavaMetaForCausalLM(ABC):
    ...
    # def get_evaclip_vision_tower(self):
    #     return self.get_model().get_vision_tower(load_model = "evaclip")
    ...
    # def encode_images_withevaclip(self, images):
    #     image_features_evaclip = self.get_model().get_vision_tower(load_model = "evaclip")(images)
    #     image_features_evaclip = self.get_model().evaclip_mm_projector(image_features_evaclip) # []
    #     print("CLIP output shape:", image_features_evaclip.shape)
    #     return image_features_evaclip
    
    def encode_images_withsiglip(self, images):
        image_features_siglip = self.get_model().get_vision_tower(load_model = "siglip")(images)
        image_features_siglip = self.get_model().siglip_mm_projector(image_features_siglip)
        # print("Siglip output shape:", image_features_siglip.shape) # []
        return image_features_siglip
    ...
    def encode_images_withconvnext(self, images):
        ...
        image_features_convnext = self.get_model().convnext_mm_projector(image_features_convnext) # []
        # print("ConvNext output shape:", image_features_convnext.shape)
        return image_features_convnext
    ...
    def prepare_inputs_labels_for_multimodal_interleaved_withsiglipdinoconvnext(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features_clip = self.encode_images_withclip(concat_images)
            image_features_siglip = self.encode_images_withsiglip(concat_images)
            image_features_dino = self.encode_images_withdino(concat_images)
            image_features_convnext = self.encode_images_withconvnext(concat_images)
            ...
            image_features_siglip = self.encode_images_withsiglip(images)
            image_features_dino = self.encode_images_withdino(images)
            image_features_convnext = self.encode_images_withconvnext(images)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                # cur_input_embeds = torch.cat([cur_input_embeds_1, image_features_clip[0:0]], dim=0)
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    # print("merged_features's shape: ", merged_features.shape)
