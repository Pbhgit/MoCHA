...
from llava.train.copy_llava_trainer import LLaVATrainer
...
class ModelArguments:
    ...
    pretrain_convnext_mm_mlp_adapter: Optional[str] = field(default=None)
    pretrain_siglip_mm_mlp_adapter: Optional[str] = field(default=None)
    # pretrain_evaclip_mm_mlp_adapter: Optional[str] = field(default=None)
    mm_projector_type: Optional[str] = field(default='linear')
    mm_use_im_start_end: bool = field(default=False)
    mm_use_im_patch_token: bool = field(default=True)
    mm_patch_merge_type: Optional[str] = field(default='flat')
    mm_vision_select_feature: Optional[str] = field(default="patch")
    # hidden_size: int = field(default=1280)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    siglip_mm_projector_lr: Optional[float] = None
    dino_mm_projector_lr: Optional[float] = None
    convnext_mm_projector_lr: Optional[float] = None
    # evaclip_mm_projector_lr: Optional[float] = None
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        keys_to_match = ['siglip_mm_projector']
        if getattr(trainer.args, "use_im_start_end", False):
            keys_to_match.extend(['embed_tokens', 'embed_in'])
        weight_to_save = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match)
        trainer.model.config.save_pretrained(output_dir)
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                mm_projector_folder = os.path.join(parent_folder, "siglip_mm_projector")
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'siglip_mm_projector.bin'))
        ...
        keys_to_match = ['convnext_mm_projector']
        if getattr(trainer.args, "use_im_start_end", False):
            keys_to_match.extend(['embed_tokens', 'embed_in'])
        weight_to_save = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match)
        trainer.model.config.save_pretrained(output_dir)
        ...
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
            if current_folder.startswith('checkpoint-'):
                mm_projector_folder = os.path.join(parent_folder, "convnext_mm_projector")
                os.makedirs(mm_projector_folder, exist_ok=True)
                torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
            else:
                torch.save(weight_to_save, os.path.join(output_dir, f'convnext_mm_projector.bin'))

        # keys_to_match = ['evaclip_mm_projector']
        # if getattr(trainer.args, "use_im_start_end", False):
        #     keys_to_match.extend(['embed_tokens', 'embed_in'])
        # weight_to_save = get_mm_adapter_state_maybe_zero_3(trainer.model.named_parameters(), keys_to_match)
        # trainer.model.config.save_pretrained(output_dir)
        ...
        # if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
        #     if current_folder.startswith('checkpoint-'):
        #         mm_projector_folder = os.path.join(parent_folder, "evaclip_mm_projector")
        #         os.makedirs(mm_projector_folder, exist_ok=True)
        #         torch.save(weight_to_save, os.path.join(mm_projector_folder, f'{current_folder}.bin'))
        #     else:
        #         torch.save(weight_to_save, os.path.join(output_dir, f'evaclip_mm_projector.bin'))
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("v1"):  # preprocess_v1
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            ###############################################################################################
            # load.jpg, .png or .gif
            image_path_jpg = os.path.join(image_folder, image_file)
            image_path_gif = os.path.splitext(image_path_jpg)[0] + '.gif'
            image_path_png = os.path.splitext(image_path_jpg)[0] + '.png'

            if os.path.exists(image_path_jpg):
                image = Image.open(image_path_jpg).convert('RGB')
            elif os.path.exists(image_path_gif):
                image = Image.open(image_path_gif).convert('RGB')
            elif os.path.exists(image_path_png):
                image = Image.open(image_path_png).convert('RGB')
            else:
                raise FileNotFoundError(f"None of the image files exist: {image_path_jpg}, {image_path_gif}, {image_path_png}.")
            #######################################################################################################
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # print("pad_token_id is: ", self.tokenizer.pad_token_id)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.tune_mm_mlp_adapter:
            ...
            for p in model.get_model().convnext_mm_projector.parameters():
                p.requires_grad = True  

            # for p in model.get_model().evaclip_mm_projector.parameters():
            #     p.requires_grad = True  
        ...
        if training_args.bits in [4, 8]:
            ...
            model.get_model().siglip_mm_projector.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().dino_mm_projector.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().convnext_mm_projector.to(dtype=compute_dtype, device=training_args.device)
            # model.get_model().evaclip_mm_projector.to(dtype=compute_dtype, device=training_args.device)


        model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = model_args.mm_use_im_start_end
        model.config.mm_projector_lr = training_args.mm_projector_lr
        model.config.siglip_mm_projector_lr = training_args.siglip_mm_projector_lr
        model.config.dino_mm_projector_lr = training_args.dino_mm_projector_lr
        model.config.convnext_mm_projector_lr = training_args.convnext_mm_projector_lr
        # model.config.evaclip_mm_projector_lr = training_args.evaclip_mm_projector_lr
